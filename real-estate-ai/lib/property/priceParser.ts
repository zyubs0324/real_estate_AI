...
function parseKoreanMoney(raw: string): number | null {
  ...
  if (cheon) total += Number(cheon[1]) * 10_000_000
  else if (manBefore) total += Number(manBefore[1]) * 10_000
  else if (manOnly) total += Number(manOnly[1]) * 10_000
